<template>
  <!--
    句子管理（移植自 frontend/templates/admin/sentence.html）
    ...
    ★ 表格已从 bootstrap-table 迁移为纯 Vue 渲染（与已完成的 CategoryManageView 同一套写法）★
      · 旧实现：$('#sentenceTable').bootstrapTable({ ajax, columns, ... })
      ...
      · 新实现：Vue 自己渲染 <thead>/<tbody>（v-for + 计算属性排序 + 自建分页）；
           行内按钮改用 @click，不再拼 HTML；数据由本组件的 load() 直接调 sentenceList。
      · 对外可见的一切保持不变：
          #toolbar / #sentenceTable / .ad-table-wrap / .ad-card* 全部保留；
          弹窗 id 与表单字段：#sentenceAddModal / #sentenceEditModal / #sentenceAddForm /
          #sentenceEditForm / #e_id / #e_content / name="content" 原样不动；
      ...
      · 排序：默认沿用后端顺序（SentenceMapper.queryData 固定 order by id desc），
        点表头才做前端排序，且仅作用于当前页（表头 title 与卡片脚已注明）。
      · 原模板表格列为 句子编号 / 内容 / 作者，没有头像列，因此不涉及 avatarUrl()。
  -->
  <div ref="rootEl" class="container-fluid ad-page">
    <AdminPageHeader
      icon="mdi mdi-format-quote-open"
      title="句子管理"
      desc="「好词佳句」前台展示的句子；新增时会自动记录创建人，删除后前台立即不再展示且不可恢复。"
    >
      <template #actions>
        <!--
          说明：#toolbar 这个 id 原样保留（回归脚本会用它）。
          历史上这里还有个 id="search" 的按钮 + doMethod.search 分支：分支读的是本页并不存在的
          搜索输入框（旧模板遗留死代码），而后端 SentenceQuery 也没有关键词过滤字段
          （SentenceMapper.queryData 固定 order by id desc）——已删除该死分支。
          按钮本身保留 id="search" 只为兼容旧引用，实际是「新增句子」入口，改为 @click 直连 openAdd()。
          原 data-toggle="modal" 无 data-target，属无效属性，一并去掉。
        -->
        <div id="toolbar" class="ad-toolbar">
          <button id="search" type="button" class="btn btn-primary btn-sm" title="新增句子" @click="openAdd()">
            <i class="mdi mdi-plus"></i> 新增句子
          </button>
        </div>
      </template>
    </AdminPageHeader>
    ...
    <div class="ad-card">
      <div class="ad-card__head">
        <h5 class="ad-card__title"><i class="mdi mdi-format-list-bulleted"></i> 句子列表</h5>
      ...
      <div class="ad-card__body ad-card__body--flush">
        <div class="ad-table-wrap">
          ...
          -->
          <table id="sentenceTable" class="table table-bordered table-hover ad-table">
            <thead>
              <tr>
                ...
                <th style="width: 120px;" class="ad-sort" :title="sortTitle('id')" @click="toggleSort('id')">
                  句子编号 <i :class="sortIcon('id')"></i>
                </th>
                <th class="ad-sort" :title="sortTitle('content')" @click="toggleSort('content')">
                  内容 <i :class="sortIcon('content')"></i>
                </th>
                <th style="width: 160px;" class="ad-sort" :title="sortTitle('createName')" @click="toggleSort('createName')">
                  作者 <i :class="sortIcon('createName')"></i>
                </th>
            ...
            <tbody>
              ...
              <tr v-else-if="!displayRows.length">
                <td colspan="5">
                  <AdminEmpty icon="mdi mdi-format-quote-close"
                              text="暂无句子，点右上角「新增句子」创建第一条句子" />
                </td>
              </tr>
    ...
    <!-- 新增表单start -->
    <div class="modal fade" id="sentenceAddModal" tabindex="-1" role="dialog" aria-labelledby="sentenceAddModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="sentenceAddModalLabel">新增句子</h4>
          </div>
          <form id="sentenceAddForm" method="post" enctype="multipart/form-data">
            <div class="modal-body">
              <div class="form-group">
                <label class="control-label">句子内容：<span class="text-danger">*</span></label>
                <input type="text" class="form-control" name="content" placeholder="请输入句子内容">
                <p class="ad-help">建议一句话表达完整含义，前台按原文展示。</p>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
              <button type="button" data-method="save" class="btn btn-primary">保存句子</button>
            </div>
          </form>
        </div>
      </div>
    ...
    <!-- 新增表单end -->
    <!-- 修改表单start -->
    <div class="modal fade" id="sentenceEditModal" tabindex="-1" role="dialog" aria-labelledby="sentenceEditModalLabel">
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="sentenceEditModalLabel">编辑句子</h4>
          </div>
          <form id="sentenceEditForm" method="post" enctype="multipart/form-data">
            <div class="modal-body">
              <input type="hidden" id="e_id" class="form-control" name="id">
              <div class="form-group">
                <label class="control-label">句子内容：<span class="text-danger">*</span></label>
                <input type="text" id="e_content" class="form-control" name="content">
                <p class="ad-help">保存后前台「好词佳句」立即更新。</p>
              </div>
            </div>
            <div class="modal-footer">
              <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
 ...
 *   sentenceList({ page, pageSize }) -> { total, rows }   分页列表（后端 order by id desc）
 *   sentenceSave({ id?, content })                        有 id 即编辑，无 id 即新增（JSON body）
 *   sentenceDelete(id)                                    即 POST /api/admin/sentence/delete { id }
 *   sentenceRestore({ id })                               删除撤销（逻辑删除恢复）
 * 新增时后端自动写入 createId / createName（见 ApiAdminController#sentenceSave）。
 *
 * 表格迁移说明：**本页已彻底移除 bootstrap-table**
 *   · 旧实现：$('#sentenceTable').bootstrapTable({ ajax, columns, pagination ... })
 *     —— 表头 / 表体 / 分页条 / 列筛选 / 刷新按钮全部由 jQuery 插件运行时拼 HTML 字符串生成，
 *        行内操作列还用 onclick="edit(id)" 拼字符串注入（XSS 面更大 + 依赖 window.edit / window.del）。
 *   · 新实现：Vue 自己渲染 <table>（表头固定 + 表体 v-for）+ 计算属性排序 + 自建分页；
 *        行内按钮改成 @click，不再拼 HTML；数据由本组件的 load() 直接调 sentenceList。
 ...
 *   请求参数（page / pageSize）与后端 ApiAdminController#sentenceList 完全一致。
 *
 * 仍然使用 jQuery 的地方（有意保留，都不属于「表格插件」）：
 *   · bootstrap 3 弹窗 #sentenceAddModal / #sentenceEditModal 的显示与隐藏；
 *   · bootstrapValidator 表单校验（#sentenceAddForm / #sentenceEditForm）；
 *   · 根节点上收窄后的 data-method 事件分发（save / editSave 两个保存按钮）。
 */
import { computed, nextTick, onBeforeUnmount, onMounted, ref } from 'vue'
import AdminPageHeader from '@/components/admin/AdminPageHeader.vue'
import AdminEmpty from '@/components/admin/AdminEmpty.vue'
import { sentenceDelete, sentenceList, sentenceRestore, sentenceSave } from '@/api/admin'
...
const ADD_MODAL = '#sentenceAddModal'
const EDIT_MODAL = '#sentenceEditModal'
const ADD_FORM = '#sentenceAddForm'
const EDIT_FORM = '#sentenceEditForm'
...
/** 排序字段：'' = 沿用后端默认顺序（id 倒序），'id' / 'content' / 'createName' = 前端排序 */
const sortKey = ref('')
/** 排序方向：'asc' | 'desc' */
 ...
 * 后端 SentenceMapper.queryData 也没有动态 order by，点表头同样只影响当页，因此行为等价。
 */
const displayRows = computed(() => {
  ...
  return list.slice().sort((a, b) => {
    ...
    // 文本列按中文拼音比较；完全相同时退回 id 倒序，保证顺序稳定
    else if (key === 'content') ret = String(a.content || '').localeCompare(String(b.content || ''), 'zh-Hans-CN')
    else ret = String(a.createName || '').localeCompare(String(b.createName || ''), 'zh-Hans-CN')
...
function sortTitle(key) {
  const label = key === 'id' ? '句子编号' : (key === 'content' ? '内容' : '作者')
  const tip = '（排序仅作用于当前页）'
...
async function load() {
  ...
  let data = null
  try {
    data = await sentenceList({ page: page.value, pageSize: pageSize.value })
  } catch (err) {
    if (disposed) return
    rows.value = []
    total.value = 0
    selectedIds.value = []
    loading.value = false
    dsToast.error((err && err.message) || '句子列表加载失败')
    return
  }
  if (disposed) return
...
/** 刷新列表：原 $("#sentenceTable").bootstrapTable('refresh') 的等价实现 */
 ...
 * 避免页面不可用）。
 * 说明：原 sentence.html 本身没有 bootstrapValidator 配置块（只有 admin/culture.html、
...
function save() {
  ...
  sentenceSave({
    content: $(ADD_FORM).find('[name=content]').val()
  }).then(() => {
    if (disposed) return
...
function editSave() {
  ...
  sentenceSave({
    ...
    content: $(EDIT_FORM).find('[name=content]').val()
  }).then(() => {
    if (disposed) return
    confirmBox({
      title: '温馨提示',
...
/** 原模板 del(id)：删除前统一二次确认（列出句子内容与影响），确认后才调用删除接口 */
function del(id) {
  const row = findRow(id)
  confirmDelete({
    name: (row && row.content || '这条句子').slice(0, 16),
    extra: '删除后「好词佳句」不再展示该句子，且不可恢复。',
...
function doDelete(id) {
  //发送ajax请求删除数据
  sentenceDelete(id).then(() => {
    if (disposed) return
    refreshList()
    dsToast.withUndo('句子已删除', async () => {
      await sentenceRestore({ id })
      if (disposed) return
      refreshList()
      dsToast.success('句子已恢复')
    })
  }).catch(err => {
    if (disposed) return
...
/**
 * data-method 分发：只保留弹窗底部两个按钮（保存句子 / 保存修改）。
 * 历史上这里还有 add / search 两个分支：
 *   · add    → 已收敛为上面的 openAdd()（按钮改为 @click 直接调用）；
 *   · search → 读的是本页并不存在的搜索输入框（旧模板遗留死代码），且后端 SentenceQuery
 *              没有关键词过滤字段（SentenceMapper.queryData 固定 order by id desc），
...
onMounted(async () => {
  ...
  if (!$ || !$.fn) {
    ...
    console.error('[SentenceManageView] jQuery 未就绪，弹窗与表单校验不可用')
    load()
    return
  }
...
<!--
  原 sentence.html 页内的 .my-container / .myLabel-content / .myText-content / .myBtn-content
